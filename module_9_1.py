...
# в списке оставить только числа
def get_nums_from_list(int_list):
    int_list = [i for i in int_list if isinstance(i, int) or isinstance(i, float)]
    return int_list

def apply_all_func(int_list, *functions):
    results = {}
    int_list = get_nums_from_list(int_list)
    for f in functions:
        # для функций max и min список не должен быть пустым
        try:
            results[f.__name__] = f(int_list)
        except ValueError as exc:
            print(f'Ошибка работы функции {f.__name__} для списка {int_list}')
    return results
