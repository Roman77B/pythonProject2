...
class Knight(threading.Thread):
    ...
    def run(self):
        print(f'{self.name} на нас напали!') # {threading.current_thread()}
        en, d = 100, 0
        while en > 0:
            en -= self.power
            d += 1
            time.sleep(1)
            print(f'{self.name}, сражается {d} день(дня)..., осталось {en} воинов') # {threading.current_thread()}
        print(f'{self.name} одержал победу спустя {d} дней(дня)!')

first_knight = Knight('Sir Lancelot', 10)
second_knight = Knight('Sir Galahad', 20)
first_knight.start()
time.sleep(0.1) # задержка между потоками, иначе они синхронно выполняются и print друг на друга накладывается
second_knight.start()
